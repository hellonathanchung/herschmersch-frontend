  ...
  render() {
    const portfolioStocks = this.props.stockList.map((userStock) => (
      <div className="ui raised link card" key={userStock.id}>
        ...
        <Button className="ve basic button" onClick={(event) => this.props.removeStockFromList(event, userStock.id)}>
          Remove Stock{" "}
        </Button>
        <Modal trigger={<Button primary>View More </Button>} header={userStock.stockInformation.name}>
          <Modal.Content>
            {<StockChart name={userStock.stockInformation.name} symbol={userStock.stockInformation.symbol} />}
            <h4 className="description">Shares: {userStock.shares}</h4>
            <h4 className="description">Initial Cost: ${userStock.initial_cost}</h4>
            <h4 style={{ color: ((this.props.currentPrice - userStock.initial_cost) * userStock.shares).toFixed(2) > 0 ? "green" : "red" }} className="description">
              Total Gain/Loss: {((this.props.currentPrice - userStock.initial_cost) * userStock.shares).toFixed(2)}
            </h4>
          </Modal.Content>
...
const mapStateToProps = (state) => {
  return { stockList: state.stockList.stockList, currentPrice: state.stocks.currentPrice };
};
